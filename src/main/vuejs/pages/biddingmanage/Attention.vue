<template>
  <div>
    <!-- 操作栏 -->
    <el-row :gutter="20" class="m10" v-if="flag=='bid'">
      <el-col :span="12">
    ...
    >
      ...
      <el-table-column label="操作" width="150" v-if="flag=='bid'">
        <template slot-scope="scope">
    ...
    <AttentionAdd v-if="isShowDialog"
...
import AttentionAdd from './AttentionAdd'
  ...
  methods: {
    ...
    deleteItem(row){
        this.$confirm(`您确定要删除标题【${row.title}】的记录吗?`, '提示', {
          ...
          Api.removeAttentionById(params).then(res=>{
              this.$notify.success({
                    title: '提示',
    ...
    loadData() {
      ...
      Api.queryBidMna(this.query).then(res => {
        if (res) {
