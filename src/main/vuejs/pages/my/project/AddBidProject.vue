  ...
  <div class="pd10">
    <el-form ref="form" :model="form" :rules="rules" label-width="160px">
      <div class="item">
        ...
        <!--投标信息 -->
        <div v-if="isBiddingProject">
          <AddBiddingProject :form="form" />
        </div>
        
      </div>
      <div class="item">
        <div class="sequence"><div class="num">2</div>流程设置：</div>
        <!-- <p class="step-title"><span class="step-icon">2</span>流程设置：</p> -->
          <div style="padding: 0 20px;">
            <el-row>
              <el-col :span="12">
                <el-form-item label="审批流程：" prop="approvalProcessId" >
                  <el-select  v-model="form.bidProjectReq.approvalProcessId" placeholder="请选择审批流程" size="medium" style="width:100%" @click.native="showSelectApproval">
                    <el-option :label="form.bidProjectReq.approvalProcessName" :value="form.bidProjectReq.approvalProcessId"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <div class="item" v-if="this.$route.query.type !== 'bid'">
        ...
        <div>
          <MaterialDetail
            ...
            :domesticMaterialDetailReqs.sync="form.domesticMaterialDetailReqs"
            :foreignMaterialDetailReqs.sync="form.foreignMaterialDetailReqs"
            :artMaterialDetailReqs.sync="form.artDetailReqs"
          ></MaterialDetail>
        </div>
      </div>-->
	  <!--艺术品项目才有预算成本
      <div class="item" v-if="this.$route.query.type === 'F'">
      ...
      <div>
        <el-row class="sub-header tar">
           <el-button type="default" @click="$router.go(-1)">返回</el-button>
            <!-- <el-button type="primary" @click="cancel">重置</el-button> -->
            <el-button type="primary" @click="onSubmit('form', 1)">提交</el-button>
            <el-button type="primary" @click="onSubmit('form', 2)">提交后继续创建</el-button>
        </el-row>
      </div>
    </el-form>
    <SelectApprovalProcess v-if="isShowProcessDialog" :isShowDialog="isShowProcessDialog" @closeDialog="closeDialog" category="2"  @confirmProcess="confirmProcess"></SelectApprovalProcess>
...
<script>
  ...
  import AddBiddingProject from './AddBiddingProject'
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        rules: {
          ...
          'bidProjectReq.budgetAmount':[
              { required: false, message: '请输入预算金额' ,trigger: 'blur' },
               { pattern: /^\+?(\d*\.\d{2})$/, message: '请输入数字，小数点保留2位。' }
          ],
          quotationAmount: [
            { required: false, message: '' ,trigger: 'blur' },
          ...
          approvalProcessId:[
            { required: false, message: '请选择审批流程' ,trigger: ['blur','change'] },
          ],
    ...
    mounted () {
        this.operateFlag = this.$route.path === '/router/editProject' ?  'edit' : 'add'
        const type = this.$route.query.type
       // 新增航材项目
       if (type === 'A') {
         ...
         // 新增投标项目；
         this.options = [...this.artOptions,...this.materialOptions,...this.dfgOptions]
    ...
    methods: {
      ...
      confirmProcess(processInfo){
        this.closeDialog()
        this.form.bidProjectReq.approvalProcessName = processInfo.name
        this.form.bidProjectReq.approvalProcessId = processInfo.id
      },
      //显示审批流程
      showSelectApproval(){
      ...
      handleSubmit(projectName,fn,form){
      	 const biddingType =this.$route.query.biddingType
      	 form.type = biddingType
