  ...
  <div>
      ...
      >
        ...
        <template slot-scope="scope" fixed="right">
           ...
           <el-button type="primary"  size="mini" v-if="scope.row.status === '0'" @click="deleteItem(scope.row)">删除</el-button>
        </template>
      </el-table-column>
      </el-table>
      <ProjectRevenueBillAdd
       v-if="isShowDialog"
...
<script>
import ProjectRevenueBillAdd from "./ProjectRevenueBillAdd"
...
export default {
  ...
  methods: {
    ...
    getDictsKeyName(row,key,val){
     ...
     udateFinance (item) {
        this.$confirm(`您确定要启用进账单位名称【${item.name}】信息吗?`, '提示', {
      ...
      udateStop (item) {
        this.$confirm(`您确定要停用进账单位名称【${item.name}】信息吗?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      deleteItem (item) {
        this.$confirm(`您确定要删除进账单位名称【${item.name}】信息吗?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
