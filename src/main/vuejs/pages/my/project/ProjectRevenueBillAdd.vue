<template>
  <el-dialog
      ...
      <el-form label-width="100px" :model="editForm" :rules="rules" ref="editForm" class="demo-form-inline">
        <el-row>
          <el-col :span="12">
                <el-form-item label="进账单位名称：" prop="name">
                  <el-select  remote :remote-method="remoteMethod" filterable v-model="editForm.name" 
                  allow-create
                   clearable placeholder="请输入进账单位名称" size="medium" style="width:100%">
  ...
  export default {
    ...
    data: function () {
      return {
        ...
        rules: {
            renown: [
              { required: true, message: '请输入进账单位名称', trigger: 'blur'},
            ],
            amount: [
            ...
            name:[
              { required: true, message: '请选择进账单位名称', trigger: 'blur'},
            ],
        },
      }
    },
    ...
    methods: {
      remoteMethod(query) {
        ...
        setTimeout(() => {
          ...
          Api.customerList(params).then(data => {
     ...
     selectBlur(e){
      this.editForm.name = e.target.value
    },
      ...
      confirm(formName){
          this.$refs[formName].validate((valid) => {
            if (valid) {
                ...
                editForm.projectCode = this.$route.query.id;
