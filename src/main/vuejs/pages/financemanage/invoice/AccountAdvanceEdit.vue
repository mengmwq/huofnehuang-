  ...
  <div>
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="160px">
      ...
      <span  style="float: right; color: #8492a6; font-size: 13px">{{ item.contractName }}</span>
            ...
            <el-col :span="12">
              <el-form-item label="申请日期：" prop="applyDate">
                <el-date-picker type="date" placeholder="请选择申请日期" v-model="form.applyDate"
                                value-format="yyyy-MM-dd HH:mm:ss" size="medium"
        ...
        <div class="dash-content" >
          <el-table class="page-table" :data="form.receiptApplyDetailReqList" 
            ...
            <el-table-column v-for="(header, index) in tableHeader" v-if="header.show" :key="index" :prop="header.prop"  :label="header.label" :sortable="header.sortable"
              ...
              <template slot-scope="scope">
                ...
                <div v-else-if="header.prop == 'number'">
                  <el-input-number v-model="scope.row.number" controls-position="right" style="width:100px;"   :min="0" @blur="calcPrice(scope.row.number, scope.$index, 'form.receiptApplyDetailReqList', 'number')"></el-input-number>
                </div>
                <div v-else-if="header.prop == 'unitPrice'">
                  <el-input-number v-model="scope.row.unitPrice" controls-position="right" style="width:100px;"  :min="0" @blur="calcPrice(scope.row.unitPrice, scope.$index, 'form.receiptApplyDetailReqList', 'unitPrice')"></el-input-number>
                </div>
        ...
        <div class="dash-content" style="padding: 0 20px;">
          <el-row>
            <el-col :span="12">
              ...
              <el-form-item v-else label="审批流程：" prop="approvalProcessId">
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      // Api.customerList({pageNum:1,pageSize:50000}).then(data => {
      ...
      if (sessionStorage.getItem('dists')) {
        ...
        if (distsObj.ReceiptType) {
          this.receiptTypeSelect = distsObj.ReceiptType.children
        }
      }
