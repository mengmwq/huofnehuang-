  ...
  <div>
    ...
    <div  v-if="typeShow=== 'detail'">
      ...
      <ImgPreview
        ...
        :isFullscreen="false"
        v-if="isShowDialogImg" @closeDialog="closeDialog"  />
    </div>
    <div v-else>
      ...
      <span style="float: right; color: #8492a6; font-size: 13px">{{ item.projectName }}</span>
              ...
              <el-col :span="8">
                <el-form-item label="合同期限：" prop="contractLimit" :rules="[rules.required]">
                  <el-date-picker v-model="form.contractLimit" type="date"
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="审批流程：" prop="definitionAppProcessId" :rules="[rules.required]">
                  <el-select  v-model="form.definitionAppProcessId" placeholder="请选择审批流程" size="medium" style="width:100%" @click.native="showSelectApproval">
                    <el-option :label="form.definitionAppProcessName" :value="form.definitionAppProcessId"></el-option>
                  </el-select>
                </el-form-item>
              </el-col>
            </el-row>
    ...
    <SelectApprovalProcess v-if="isShowProcessDialog" :isShowDialog="isShowProcessDialog" @closeDialog="closeDialog" category="1"  @confirmProcess="confirmProcess"></SelectApprovalProcess>
  ...
  export default {
    ...
    methods: {
      ...
      confirmProcess(processInfo){
        this.closeDialog()
        this.form.definitionAppProcessName = processInfo.name
        this.form.definitionAppProcessId = processInfo.id
      },
      //显示审批流程
      showSelectApproval(){
      ...
      /** 添加数据 */
      add1: function () {
      this.form.type = this.$route.query.contractsType
        Api.addContract(this.form).then(data => {
          if (data) {
            ...
            // 跳转回去
            this.$router.go(-1)
            //this.$router.push({path: '/router/contractsList'})
          }
        })
      },
      addItem () {
