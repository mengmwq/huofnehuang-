<template>
  <div>
    ...
    <el-row :gutter="20" class="m10">
      <el-col :span="4">
        <el-button type="primary" size="medium" v-if="$route.path==='/router/contactsList'" v-showBtn data-authorityType="add-btn" @click="addContracts" icon="el-icon-plus">新增合同</el-button>
        <el-button type="primary" size="medium" v-else @click="addContracts" icon="el-icon-plus">新增合同</el-button>
      </el-col>
      <el-col :span="20" class="text-right" style="float: right;">
        ...
        <el-input size="medium" clearable placeholder="合同名称/项目编号/标题" icon="search" v-model="query.titleOrProjectCode" @keyup.enter.native="loadData" style="width: 200px;"></el-input>
      ...
      <el-table-column v-for="(header, index) in tableHeader" v-if="header.show" :key="index" :fixed="header.fixed" :prop="header.prop"  :label="header.label" :sortable="header.sortable"
        ...
        <template slot-scope="scope">
          ...
          <div v-else-if="header.prop === 'contractCode'">
            <router-link :to="{path: '/router/AddContracts', query: {id: scope.row.id, type: 'detail'}}">
