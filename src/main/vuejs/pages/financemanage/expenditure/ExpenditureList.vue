<template>
  <div>
    ...
    <el-row :gutter="20" class="m10">
      <el-col :span="4">
        <el-button type="primary" size="medium" @click="newPayApply" v-showBtn data-authorityType="add-btn" icon="el-icon-plus">申请付款</el-button>
      </el-col>
      <el-col :span="20" class="text-right" style="float: right;">
        ...
        <el-input size="medium" clearable placeholder="请输入项目/合同编号、客户名称" icon="search" v-model="query.searchStr" @keyup.enter.native="loadData" style="width: 290px;"></el-input>
    ...
    >
       ...
       :prop="header.prop"  :label="header.label" :sortable="header.sortable" :fixed="header.fixed"
        ...
        <template slot-scope="scope">
          ...
          <div v-else-if="header.prop === 'id'">
            <router-link :to="{path: '/router/paymentDetail', query: {id: scope.row.id}}">
             {{scope.row.id}}
          </router-link>
          </div>
      ...
      <el-table-column label="操作" width="150" fixed="right">
        <template slot-scope="scope">
          ...
          <router-link :to="{path: '/router/editPaymentApply', query: {id: scope.row.id}}" v-showBtn data-authorityType="edit-btn">
            <el-button type="primary" size="mini">编辑</el-button>
          </router-link>
          <!--<el-button type="primary" size="mini" @click="deleteInvoice(scope.row)">删除</el-button>-->
  ...
  export default {
    ...
    methods: {
      ...
      loadData (pageNum=1) {
        ...
        Api.payPageList(this.query).then((res) => {
          if (res) {
      ...
      newPayApply: function () {
        this.$router.push({path: '/router/addPaymentApply'})
      },
      /**
       * 付款
       */
