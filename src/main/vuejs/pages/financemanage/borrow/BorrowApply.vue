          ...
          <el-row>
            <el-col :span="12">
              ...
              <el-form-item v-else label="项目编码：" prop="projectCode">
                ...
                remote :remote-method="remoteMethod" clearable placeholder="项目名称/编号/客户名称" size="medium" style="width: 100%;">
                  <el-option
      ...
      <span style="float: right; color: #8492a6; font-size: 13px">{{ item.projectCode }}</span>
            ...
            <el-col :span="12">
              <el-form-item label="合同编号：" prop="contractId">
                ...
                <el-select v-else filterable clearable style="width:100%" v-model="form.contractId" placeholder="合同号">
                  <el-option v-for="(item, index) in contractList" :key="index" :label="item.contractName" :value="item.id">
      ...
      <span  style="float: right; color: #8492a6; font-size: 13px">{{ item.contractName }}</span>
            ...
            <el-col :span="12">
              <el-form-item label="申请人：" prop="applyerId">
                <el-select filterable clearable v-model="form.applyerId" clearable placeholder="申请人" size="medium" style="width: 100%;">
            ...
            <el-col :span="12">
              <el-form-item label="申请部门：" prop="orgId">
                <el-select filterable v-model="form.orgName" placeholder="申请部门" size="medium" style="width: 100%;">
            ...
            <el-col :span="12">
              <el-form-item label="申请日期：" prop="applyDate">
                <el-date-picker v-model="form.applyDate" type="datetime" style="width: 100%"
                                value-format="yyyy-MM-dd HH:mm:ss" format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期"></el-date-picker>
              </el-form-item>
            </el-col>
            <el-col :span="12">
           ...
           <el-col :span="12">
              <el-form-item label="领用人：" prop="payeeId">
                <el-select filterable v-model="form.payeeId" clearable placeholder="申请人" size="medium" style="width: 100%;">
                  <el-option v-for="(item, index) in userList" :key="index" :label="item.uName" :value="item.uId"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
        ...
        <div class="dash-content" style="padding: 0 4%;">
          ...
          <el-table class="page-table mb10 " :data="listData" show-summary :summary-method="getSummaries" 
            ...
            <el-table-column v-for="(header, index) in tableHeader" v-if="header.show" :key="index" :prop="header.prop"  :label="header.label" :sortable="header.sortable"
              ...
              <template slot-scope="scope">
                ...
                <div v-else-if="header.prop == 'rechargingAmount'">
                  <el-input-number v-model="scope.row.rechargingAmount" controls-position="right" style="width:100px;" :min="0" @change="calcPrice(scope.row.rechargingAmount, scope.$index, 'listData', 'rechargingAmount')" @blur="calcPrice(scope.row.rechargingAmount, scope.$index, 'listData', 'rechargingAmount')"></el-input-number>
                </div>
                <div v-else-if="header.prop == 'cashAmount'">
                  <el-input-number v-model="scope.row.cashAmount" controls-position="right" style="width:100px;" :min="0" @change="calcPrice(scope.row.cashAmount, scope.$index, 'listData', 'cashAmount')" @blur="calcPrice(scope.row.cashAmount, scope.$index, 'listData', 'cashAmount')"></el-input-number>
                </div>
                <div v-else-if="header.prop == 'amount'">
                  <el-input-number v-model="scope.row.amount" controls-position="right" style="width:100px;" :min="0" @change="calcAmountPrice(scope.row.amount, scope.$index, 'listData', 'amount')" @blur="calcAmountPrice(scope.row.amount, scope.$index, 'listData', 'amount')"></el-input-number>
                </div>
          ...
          <el-row>
            <el-col :span="12">
              ...
              <el-form-item label="审批流程：" prop="approvalProcessId">
                 <span v-if="$route.query.flag=='myApply'">{{form.approvalName}}</span>
                <el-select v-else filterable v-model="form.approvalProcessId" clearable placeholder="请选择审批流程" size="medium" style="width:100%">
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.operate = this.$route.path === '/router/editBorrowApply'? 'edit' : 'add'
      // 项目列表
      this.remoteMethod()
    ...
    methods: {
      ...
      loadData () {
        ...
        Api.loanDetail({id: this.$route.query.id}).then(data => {
          if (data) {
           this.form = data
           this.form.applyDate = data.applyDate ? moment(data.applyDate).format('YYYY-MM-DD HH:mm:ss') : ''
