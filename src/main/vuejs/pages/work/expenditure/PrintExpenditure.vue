  ...
  <div v-if="form" class="printBg">
    ...
    <table class="printTable">
      <tbody>
        ...
        <tr>
          <th>行政事项</th>
          <td colspan="4">{{form.content}}</td>
          <th>合同号</th>
          <td colspan="4">{{form.contractCode}}</td>
        </tr>
        <tr>
          <th>供应商类型</th>
          <td colspan="4"><span v-if="form.attached && form.attached.SupplierType"> {{form.attached.SupplierType[form.supplierType].cnInfo}}</span></td>
          <th>供应商名称</th>
          <td colspan="4">{{form.customName}}</td>
        </tr>
        <tr>
          <th>币种</th>
          <td colspan="4"><span v-if="form.attached && form.attached.Currency">{{form.attached.Currency[form.currency].cnInfo}}</span></td>
          <th>汇率</th>
        ...
        <tr v-if="form && form.applyAttachmentReqs.length>0">
          <th>附件</th>
          <td colspan="9">
            {{getAttachmentList(form.applyAttachmentReqs)}}
          </td>
        </tr>
      </tbody>
    </table>
