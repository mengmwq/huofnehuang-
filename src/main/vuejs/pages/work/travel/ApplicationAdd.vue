  ...
  <div>
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="130px">
      ...
      <div class="print-data">
              ...
              <el-form-item label="项目编码：" prop="projectCode">
                ...
                >
                  <el-option
                    ...
                    :key="index"
                    @click.native="getloanApplyId(item)"
          ...
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="预借款项：" prop="item">
                <el-input v-model="form.item" placeholder="只能输入数字，小数点后最多两位"  type="number"  min="0"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
        ...
        <p class="step-title">
          <span class="step-icon">2</span>交通明细：
        </p>
        <div class="dash-content pd10">
          <el-table
            ...
            :data="trafficData"
            show-summary
            :summary-method="getSummariesTraffice"
            stripe
          ...
          >
            <el-table-column
              v-for="(header, index) in trafficHeader"
              v-if="header.show"
              :key="index"
              :prop="header.prop"
              :label="header.label"
            ...
            >
              <template slot-scope="scope">
                <div v-if="header.prop == 'trafficTypeId'">
                  <el-select
                    v-model="scope.row.trafficTypeId"
                    placeholder="请选择工具类型"
                    style="width: 100%;"
                  >
                    <el-option
                ...
                <div v-else-if="header.prop == 'amount'">
                  ...
                  <el-input-number
                    ...
                    style="width:100px;"
                    :min="0"
                    @change="calcPrice(scope.row.amount, scope.$index, 'trafficData', 'amount')"
                    @blur="calcPrice(scope.row.amount, scope.$index, 'trafficData', 'amount')"
                  ></el-input-number>
                </div>
            ...
            <el-table-column align="left" width="60">
              <template slot="header">
                <el-button size="mini" type="primary" icon="el-icon-plus" @click="addTrafficItem"></el-button>
              </template>
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  icon="el-icon-minus"
                  @click="() => {trafficData.splice(scope.$index, 1)}"
                ></el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <p class="step-title">
          <span class="step-icon">3</span>住宿明细：
        </p>
        <div class="dash-content pd10">
          <el-table
            class="page-table"
            :data="stayData"
            show-summary
            :summary-method="getSummariesStay"
            stripe
            highlight-current-row
            v-loading="listLoading"
            :cell-style="{color:'#333'}"
          >
            <el-table-column
              v-for="(header, index) in listStay"
              v-if="header.show"
              :key="index"
              :prop="header.prop"
              :label="header.label"
              ...
              <template slot-scope="scope">
                ...
                <div v-if="header.prop == 'amount'">
                  <el-input-number
                    ...
                    style="width:100px;"
                    :min="0"
                    @change="calcTotalAmount(scope.row, scope.$index, stayData),calcPrice(scope.row.amount, scope.$index, 'stayData', 'amount')"
                    @blur="calcTotalAmount(scope.row, scope.$index, stayData),calcPrice(scope.row.amount, scope.$index, 'stayData', 'amount')"
                  ></el-input-number>
                ...
                <div v-if="header.prop == 'roomNum'">
                  <el-input-number
                    ...
                    @change="calcTotalAmount(scope.row, scope.$index, stayData)"
                    @blur="calcTotalAmount(scope.row, scope.$index, stayData)"
                    style="width:100px;"
                    controls-position="right"
                    :min="0"
                  ></el-input-number>
            ...
            <el-table-column align="left" width="60">
              <template slot="header">
                <el-button size="mini" type="primary" icon="el-icon-plus" @click="addStayItem"></el-button>
              </template>
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  icon="el-icon-minus"
                  @click="() => {stayData.splice(scope.$index, 1)}"
                ></el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <p class="step-title">
          <span class="step-icon">4</span>餐费明细：
        </p>
        <div class="dash-content pd10">
          <el-table
            class="page-table"
          ...
          >
            <el-table-column
              v-for="(header, index) in listMeal"
              v-if="header.show"
              :key="index"
              :prop="header.prop"
              :label="header.label"
              ...
              <template slot-scope="scope">
                ...
                <div v-if="header.prop == 'amount'">
                  ...
                  <el-input-number
                    ...
                    controls-position="right"
                    :min="0"
                    @change="calcMealTotal(scope.row, scope.$index, mealData),calcPrice(scope.row.amount, scope.$index, 'mealData', 'amount')"
                    @blur="calcMealTotal(scope.row, scope.$index, mealData),calcPrice(scope.row.amount, scope.$index, 'mealData', 'amount')"
                  ></el-input-number>
                </div>
                <div v-if="header.prop == 'subsidyAmount'">
                  <el-input-number
                    ...
                    controls-position="right"
                    :min="0"
                    @change="calcMealTotal(scope.row, scope.$index, mealData),calcPrice(scope.row.subsidyAmount, scope.$index, 'mealData', 'subsidyAmount')"
                    @blur="calcMealTotal(scope.row, scope.$index, mealData),calcPrice(scope.row.subsidyAmount, scope.$index, 'mealData', 'subsidyAmount')"
                  ></el-input-number>
                </div>
                <div
                  v-if="header.prop == 'totalAmount'"
                >{{scope.row.amount + scope.row.subsidyAmount}}</div>
                <div v-if="header.prop == 'remarks'">{{scope.row.remarks}}</div>
              </template>
            </el-table-column>
            <el-table-column align="left" width="60">
              <template slot="header">
                <el-button size="mini" type="primary" icon="el-icon-plus" @click="addMealItem"></el-button>
              </template>
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  icon="el-icon-minus"
                  @click="() => {mealData.splice(scope.$index, 1)}"
                ></el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <p class="step-title">
          <span class="step-icon">5</span>补贴明细：
        </p>
        <div class="dash-content pd10">
          <el-table
            class="page-table"
            :data="subsidyData"
            show-summary
            :summary-method="getSummariesSubsidy"
            stripe
            highlight-current-row
            v-loading="listLoading"
            :cell-style="{color:'#333'}"
          >
            <el-table-column
              v-for="(header, index) in listSubsidy"
              v-if="header.show"
              :key="index"
              :prop="header.prop"
              :label="header.label"
              ...
              <template slot-scope="scope">
                ...
                <div v-if="header.prop == 'days'">
                  ...
                  <el-input-number
                    ...
                    controls-position="right"
                    style="width:100px;"
                    @change="calcSubsidyTotal(scope.row, scope.$index, subsidyData),calcPrice(scope.row.days, scope.$index, 'subsidyData', 'days')"
                    @blur="calcSubsidyTotal(scope.row, scope.$index, subsidyData),calcPrice(scope.row.days, scope.$index, 'subsidyData', 'days')"
                  ></el-input-number>
                </div>
                <div v-if="header.prop == 'subsidyAmount'">
                  <el-input-number
                    ...
                    style="width:100px;"
                    :min="0"
                    @change="calcSubsidyTotal(scope.row, scope.$index, subsidyData),calcPrice(scope.row.subsidyAmount, scope.$index, 'subsidyData', 'subsidyAmount')"
                    @blur="calcSubsidyTotal(scope.row, scope.$index, subsidyData),calcPrice(scope.row.subsidyAmount, scope.$index, 'subsidyData', 'subsidyAmount')"
                  ></el-input-number>
                </div>
                <div v-if="header.prop == 'otherSubsidyAmount'">
                  <el-input-number
                    v-model="scope.row.otherSubsidyAmount"
                    controls-position="right"
                    style="width:100px;"
                    :min="0"
                    @change="calcSubsidyTotal(scope.row, scope.$index, subsidyData),calcPrice(scope.row.otherSubsidyAmount, scope.$index, 'subsidyData', 'otherSubsidyAmount')"
                    @blur="calcSubsidyTotal(scope.row, scope.$index, subsidyData),calcPrice(scope.row.otherSubsidyAmount, scope.$index, 'subsidyData', 'otherSubsidyAmount')"
                  ></el-input-number>
                </div>
                <div v-if="header.prop == 'totalAmount'">
            ...
            <el-table-column align="left" width="60">
              <template slot="header">
                <el-button size="mini" type="primary" icon="el-icon-plus" @click="addSubSidyItem"></el-button>
              </template>
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  icon="el-icon-minus"
                  @click="() => {subsidyData.splice(scope.$index, 1)}"
                ></el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <p class="step-title">
          <span class="step-icon">6</span>其他明细：
        </p>
        <div class="dash-content pd10">
          <el-table
            class="page-table"
          ...
          >
            <el-table-column
              v-for="(header, index) in listOther"
              v-if="header.show"
              :key="index"
              :prop="header.prop"
              :label="header.label"
              ...
              <template slot-scope="scope">
                ...
                <div v-else-if="header.prop == 'amount'">
                  ...
                  <el-input-number
                    ...
                    style="width:100px;"
                    :min="0"
                    @change="calcOtherTotalAmount(scope.row, scope.$index, otherData),calcPrice(scope.row.amount, scope.$index, 'otherData', 'amount')"
                    @blur="calcOtherTotalAmount(scope.row, scope.$index, otherData),calcPrice(scope.row.amount, scope.$index, 'otherData', 'amount')"
                  ></el-input-number>
                </div>
                <div v-else-if="header.prop == 'num'">
                  <el-input-number
                    ...
                    style="width:100px;"
                    :min="0"
                    @change="calcOtherTotalAmount(scope.row, scope.$index, otherData),calcPrice(scope.row.num, scope.$index, 'otherData', 'num')"
                    @blur="calcOtherTotalAmount(scope.row, scope.$index, otherData),calcPrice(scope.row.num, scope.$index, 'otherData', 'num')"
                  ></el-input-number>
                </div>
            ...
            <el-table-column align="left" width="60">
              <template slot="header">
                <el-button size="mini" type="primary" icon="el-icon-plus" @click="addOtherItem"></el-button>
              </template>
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  icon="el-icon-minus"
                  @click="() => {otherData.splice(scope.$index, 1)}"
                ></el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <p class="step-title">
          <span class="step-icon">8</span>单据上传：
        </p>
        <div class="dash-content pd10">
        ...
        <p class="step-title">
          <span class="step-icon">9</span>选择抄送人：
        </p>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        applyer: [{ required: true, message: "不为空", trigger: "change" }],
        item: [
        ...
        projectCode: [{ required: false, message: "不为空", trigger: "blur" }],
        loanApplyId: [{ required: false, message: "不为空", trigger: "blur" }],
      },
      trafficData: [],
      stayData: [],
      mealData: [],
  ...
  watch: {
    ...
    "form.amount": function () {
      if (this.form.item) {
        this.form.actualPay =
          parseFloat(this.form.amount) - parseFloat(this.form.item);
      } else {
  ...
  mounted() {
    ...
    this.$set(this.form, "applyer", this.userInfo.uName);

    console.log(this.form);
    this.form.orgName = this.userInfo.orgName;
    this.form.positionName = this.userInfo.positionName;
    ...
    Api.loanPage({ pageNum: 1, pageSize: 50000 }).then((data) => {
      console.log(data, "bdddddddddddd");
      if (data) {
  ...
  methods: {
    ...
    getloanApplyId(item) {
      ...
      this.$set(this.form,'loanApplyId','')
      // console.log(item, "dd");
    ...
    sumAmount() {
      ...
      const amount = parseFloat(
        trafficeAmount + stayAmount + mealAmount + subsidyAmount + othersAmount
      ).toFixed(2);
      this.form.amount = amount;
      this.form.actualPay = amount - (this.form.item || 0);
    },
    //交通合计
      ...
      columns.forEach((column, index) => {
        ...
        if (
          ...
          (column.property == "totalAmount" ||
            column.property == "days" ||
            column.property == "residents" ||
            column.property == "roomNum" ||
            column.property == "amount")
        ) {
          sums[index] = values.reduce((prev, curr) => {
            const value = Number(curr);
    ...
    getSummariesSubsidy(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (
          ...
          (column.property == "totalAmount" ||
            column.property == "days" ||
            column.property == "subsidyAmount" ||
            column.property == "otherSubsidyAmount")
        ) {
          sums[index] = values.reduce((prev, curr) => {
            const value = Number(curr);
            if (!isNaN(value)) {
    ...
    calcTotalAmount(item, index, stayData) {
      if (
        ...
        (item.roomNum || item.roomNum === 0) &&
        (item.days || item.days === 0)
      ) {
        stayData[index].totalAmount = (
          parseFloat(item.amount) *
          parseInt(item.roomNum) *
          parseFloat(item.days)
        ).toFixed(2);
      }
    },
    /** 餐费管理计算总金额 */
    calcMealTotal(item, index, mealData) {
      if (
        ...
        mealData[index].totalAmount = (
          parseFloat(item.subsidyAmount) + parseFloat(item.amount)
        ).toFixed(2);
      }
    },
    /** 计算补贴总金额 */
    calcSubsidyTotal(item, index, subsidyData) {
      if (
        (item.days || item.days === 0) &&
        (item.subsidyAmount || item.subsidyAmount === 0) &&
        (item.otherSubsidyAmount || item.otherSubsidyAmount === 0)
      ) {
        subsidyData[index].totalAmount = (
          parseFloat(item.days) *
          (parseFloat(item.subsidyAmount) + parseFloat(item.otherSubsidyAmount))
        ).toFixed(2);
      }
      if (
        (item.days || item.days === 0) &&
        (item.subsidyAmount || item.subsidyAmount === 0) &&
        !item.otherSubsidyAmount
      ) {
        subsidyData[index].totalAmount = (
          parseFloat(item.days) *
          (parseFloat(item.subsidyAmount) + parseFloat(item.otherSubsidyAmount))
    ...
    calcOtherTotalAmount(item, index, otherData) {
      if ((item.amount || item.amount === 0) && (item.num || item.num === 0)) {
        otherData[index].totalAmount = (
          parseFloat(item.amount) * parseFloat(item.num)
        ).toFixed(2);
      }
    },
    addTrafficItem() {
