...
export default {
  ...
  methods: {
    ...
    handleCommand (item) {
      ...
      if (flag === 0) {
        ...
        this.$router.push({
          ...
          // 启用
        this.submitAgreementStatus(row, '启用')
      } else if (flag === 2) {
          // 停用
        console.log(item, flag, 'close')
        this.submitAgreementStatus(row, '停用')
      } else if (flag == 3) {
        const path = row.type == 1 ? 'addAgreement' : 'addContract'
    ...
    editItem (flag, row) {
      ...
      let path = 'addMarketDevelopment'
      if (flag === 'edit') {
        path = 'editMarketDevelopment'
