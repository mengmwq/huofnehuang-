<template>
  <div>
    <el-form label-position="right" :model="form" :rules="rules" ref="form" label-width="130px">
      <!-- 打印内容开始 -->
      <div class="print-data pd20">
        <!-- 基础信息 -->
        <!-- <p class="step-title">
          <span class="step-icon">1</span>基本信息：
        </p>-->
        <div _class="dash-content">
          <el-row>
            <el-col :span="12">
            ...
            <!-- <el-col :span="12">
              <el-form-item label="途径地" prop="pathway ">
                <el-input v-model="form.pathway " maxlength="20"></el-input>
              </el-form-item>
            </el-col> -->
            <el-col :span="12">
      ...
      <div class="footer m10">
        ...
        <el-button type="primary" size="medium" @click="confirm('form', 0)">提 交</el-button>
        <!-- <el-button
          type="primary"
          size="medium"
...
export default {
  ...
  mounted() {
    this.operate =
      this.$route.path === "/router/editOilCardApplication" ? "edit" : "add";
  ...
  methods: {
    ...
    loadData() {
      ...
      Api.getGasolineById({ id: this.$route.query.id }).then((res) => {
        if (res) {
          this.form = res;
        }
      });
    ...
    confirm() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          let form = this.form;
          const fn = form.id ? "updateGasolineById" : "addGasoline";
          const msg = form.id ? "编辑油卡申请成功" : "添加油卡申请成功";
          Api[fn](form)
            .then((res) => {
              this.$notify.success({
                title: "提示",
    ...
    goBack(flag) {
      const path =
        flag === "add" ? "oilCardApplication" : "addOilCardApplication";
      this.$router.push({
        path: "/router/" + path,
      });
    },
