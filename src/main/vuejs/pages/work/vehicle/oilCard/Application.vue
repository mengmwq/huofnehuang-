<template>
  <div class="mt10">
    ...
    <el-row :gutter="20"  v-if="flag==='list'">
      <el-col :span="10">
        <el-button type="primary" size="small" @click="newPayApply" data-authorityType="add-btn">申请油卡</el-button>
    ...
    >
     <el-table-column v-if="flag==='list'" type="selection" width="55"></el-table-column>
      ...
      :prop="header.prop"  :label="header.label" :sortable="header.sortable" :fixed="header.fixed"
        ...
        <template slot-scope="scope">
          ...
          <!-- <div v-else-if="header.prop === 'registrationNumber'">
            <router-link :to="{path: '/router/oilCardApplicationDetails', query: {id: scope.row.id}}">
      ...
      <el-table-column label="操作" width="80" fixed="right" v-if="flag==='list'">
        <template slot-scope="scope">
          <router-link v-if="flag==='list'" :to="{path: '/router/editOilCardApplication', query: {id: scope.row.id}}"  data-authorityType="edit-btn">
            <el-button type="primary" size="mini">编辑</el-button>
          </router-link>
          <!-- <router-link v-if="flag==='details'" :to="{path: '/router/oilCardApplicationDetails', query: {id: scope.row.id}}"  data-authorityType="edit-btn">
  ...
  export default {
    ...
    data () {
      return {
        ...
        tableHeader: [   
           {prop: 'registrationNumber',label: '车牌号',width:'100',fixed:'left',show: true},
            {prop: 'projectName',label: '项目名称',width:'240',fixed:'left',show: true}, 
            {prop: 'amount',label: '金额',show: true}, 
            {prop: 'userName',label: '申请人',show: true}, 
            {prop: 'createDt',label: '申请时间 ',width:'120', show: true}, 
    ...
    methods: {
      ...
      deleteItem(){
        ...
        this.$confirm(`确认删除车牌号【${nameStr}】的油卡申请吗？`, '提示', {
          type: 'warning'
        }).then(() => {
          ...
          Api.deleteOilCardApplication(params).then((res) => {
            if(this.listData.length==selected.length){
                this.query.pageNum =1
            }
            this.loadData();
            this.$message({showClose:true, message:`删除申请【${nameStr}】成功`, type:'success'});
          });
        }).catch(() => {
      ...
      onSubmit(){
        ...
        Api.submitOilcardApplication(params).then(res=>{
      ...
      loadData (pageNum=1) {
        ...
        const fn = flag ==='details' ? 'getOilCardApplicationByApprovalId':'getOilCardApplication'
      ...
      newPayApply: function () {
        this.$router.push({path: '/router/addOilCardApplication'})
      },
