<template>
  <div class="mt10">
    ...
    <el-row :gutter="20"  v-if="flag==='list'">
      <el-col :span="10">
        <el-button type="primary" size="small" @click="newPayApply" data-authorityType="add-btn">申请发票</el-button>
    ...
    >
     <el-table-column v-if="flag==='list'" type="selection" width="55"></el-table-column>
      ...
      :prop="header.prop"  :label="header.label" :sortable="header.sortable" :fixed="header.fixed"
        ...
        <template slot-scope="scope">
          ...
          <div v-else-if="header.prop === 'id'">
            <router-link :to="{path: '/router/invoiceApplicationDetails', query: {id: scope.row.id}}">
             {{scope.row.id}}
          </router-link>
          </div>
      ...
      <el-table-column label="操作" width="80" fixed="right">
        <template slot-scope="scope">
          <router-link v-if="flag==='list'" :to="{path: '/router/editInvoiceApplication', query: {id: scope.row.id}}"  data-authorityType="edit-btn">
            <el-button type="primary" size="mini">编辑</el-button>
          </router-link>
          <router-link v-if="flag==='details'" :to="{path: '/router/invoiceApplicationDetails', query: {id: scope.row.id, targetType:scope.row.targetType}}"  data-authorityType="edit-btn">
    ...
    methods: {
      ...
      deleteItem(){
        ...
        this.$confirm(`确认删除编号【${ids}】的申请吗？`, '提示', {
          ...
          Api.deleteInvoiceApplication(params).then((res) => {
      ...
      onSubmit(){
        ...
        Api.submitInvoiceApplication(params).then(res=>{
      ...
      loadData (pageNum=1) {
        ...
        const fn = flag ==='details' ? 'receiptApplyApiByApplyIdList':'getInvoiceAppliaction'
