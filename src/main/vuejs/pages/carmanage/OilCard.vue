<template>
  <div>
    ...
    <el-row :gutter="20" class="m10">
      <el-col :span="12">
        <el-button type="primary"  size="small" @click="oilApply('add')">油卡申请</el-button>
        <!-- <el-button type="primary"  size="small" @click="reimbursementApply">报销申请</el-button> -->
          ...
          <template slot-scope="scope">
            <span v-if="scope.row.ctUId===userInfo.uId">
              ...
              <span v-if="scope.row.approvalStatus==4 || scope.row.approvalStatus==0">
                  <el-button  type="primary" size="mini" @click="oilApply('edit',scope.row)">编辑</el-button>
...
<script>
  ...
  import OilCardApply from './OilCardApply'
  ...
  export default {
    ...
    deleteCar (i) {
        this.$confirm('您确定要删除该信息吗?', '提示', {
          ...
          Api.deleteGasolineById(param).then(() => {
            this.loadData()
            this.$notify.success({
              title: '提示',
              message: '删除成功'
      ...
      /**重新提交**/
      newSubmit (item) {
        this.$confirm('您确定要重新提交【' + item.projectName + '】 油卡申请吗?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      loadData () {
        ...
        Api.GasolineDetailList(this.query).then(data => {
          if (data) {  
