    ...
    <div>
        ...
        <el-table class="page-table"  v-if="listPage" :data="listPage.result"  highlight-current-row v-loading="listLoading"
              ...
              <el-table-column label="操作" width="100" fixed="right">
                <template slot-scope="scope">
                  <el-button type="primary" size="mini" v-if="scope.row.status==='0'" @click="closeCase(scope.row)">
                          结案
                      </el-button>
                </template>
              </el-table-column>
   ...
   //警示提醒 一个月 红色行，二个月 黄色行
    ...
    closeCase(row){
       ...
       this.$confirm(`此操作将报关单号【${customsNo}】结案, 是否继续?`, '提示', {
          ...
          Api.updateDeposit(params).then(res=>{
             this.$notify({
              title: '提示',
              type: 'success',
              message: '结案操作成功!'
    ...
    loadData (pageNum=1) {
      ...
      Api.getDepositPage(this.query).then((res) => {
        if (res) {
          this.listPage = res
        }
      })
    ...
    exportExcel () {
      if (this.listPage.result.length > 0) {
        ...
        Api.depositListExport(params).then(res => {
         // let blob = new Blob([res], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;'})
          let blob = new Blob([res], {type: 'application/vnd.ms-excel;'})
          const fileName = '保证金.xls'
