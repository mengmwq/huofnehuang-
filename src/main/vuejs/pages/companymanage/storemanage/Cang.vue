    ...
    <div>
        <el-row class="mb10">
            <el-col :span="24" class="text-left">
                <el-button type="primary" size="small" v-showBtn data-authorityType="addCang-btn" icon="el-icon-plus" @click="addStoreDialog=true;operateFlag = 'add'">新增仓</el-button>
            </el-col>
        </el-row>
        ...
        >
            ...
            <el-table-column label="操作" width="100" fixed="right">
                <template slot-scope="scope">
                    <el-button type="primary" size="mini" v-showBtn data-authorityType="editCang-btn" @click="editCang(scope.row)">编辑</el-button>
                </template>
            </el-table-column>
        </el-table>
      ...
      <el-dialog
          :close-on-click-modal="canClick"
            :title="operateFlag === 'edit' ? '编辑仓' : '新增仓'"
          ...
          <div>
              <el-form :model="addForm" ref="addForm" :rules="rules" label-width="100px">
                ...
                <el-form-item label="仓位编号:" prop="code">
                  <el-input v-model="addForm.code"></el-input>
                </el-form-item>
                <el-form-item label="仓位类型:">
                  <el-input v-model="addForm.type"></el-input>
                </el-form-item>
                <el-form-item label="仓位长宽高:">
 ...
 export default {
   ...
   data () {
     return {
       ...
       operateFlag: '',
       tableHeader: [
        {prop: 'code', label: '仓位编号', width: '100', show: true},
        {prop: 'name', label: '仓位名称', width: '100', show: true},
        {prop: 'type', align: 'center', label: '仓位类型', show: true},
        {prop: 'size', align: 'center', label: '仓位长宽高（m）', width: '180', show: true},
        {prop: 'remarks', align: 'center', label: '备注', show: true}
       ],
       rules: {
         name: [
           { required: true, message: '请输入仓名称', trigger: 'blur'},
         ],
         code: [
   ...
   methods: {
     ...
     loadData () {
       ...
       Api.housePageList(this.query).then((res) => {
         if (res) {
       ...
       this.$refs[formName].validate((valid) => {
         if (valid) {
           if (this.operateFlag === 'edit') {
             Api.updateHouseInfoById(this.addForm).then((res) => {
               if (res) {
                 this.$notify.success({
                   title: '提示',
                   message: '仓修改成功'
             ...
             Api.addHouseInfo(this.addForm).then((res) => {
               if (res) {
                 this.$notify.success({
                   title: '提示',
                   message: '仓添加成功'
