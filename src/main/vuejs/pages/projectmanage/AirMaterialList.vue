    ...
    <div>
      ...
      <el-row class="m10">
        <el-col :span="6" class="text-left">
          ...
          <router-link :to="{ path: '/router/addAirProject', query: {type: 'A'}}" v-showBtn data-authorityType="createHangCai-btn">
            <el-button type="primary" size="medium">新增航材项目</el-button>
          </router-link>
          <router-link :to="{ path: '/router/AddDfgProject', query: {type: 'DF'}}" v-showBtn data-authorityType="createMian-btn">
      ...
      <el-table class="page-table" :data="listData" stripe highlight-current-row v-loading="listLoading" :cell-style="{color:'#333'}"
        ...
        <template v-for="(header, index) in tableHeader">
          <el-table-column  v-if="header.show" :key="index" :prop="header.prop" :fixed="header.fixed" :label="header.label" :sortable="header.sortable"
          ...
          <template slot-scope="scope">
            ...
            <div v-else-if="header.prop == 'arriveDate'">
              <span>{{scope.row.arriveDate | DateFormat}}</span>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      loadData (pageNum=1) {
        ...
        Api.gerProjcetMaterialList(this.query).then((res) => {
          if (res) {
