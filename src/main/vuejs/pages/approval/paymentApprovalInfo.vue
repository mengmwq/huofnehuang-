<template>
    <div>
        ...
        <div style="padding: 0 4%;">
            <el-table class="page-table" :data="listData" show-summary :summary-method="getSummaries" stripe highlight-current-row :cell-style="{color:'#333'}"
            >
                ...
                <div v-if="header.prop == 'feeTypeId'">
                  {{scope.row.attached.FeeType[scope.row.feeTypeId].cnInfo}}
                </div>
                <div v-else-if="header.prop == 'conversion'">
                  {{parseFloat(scope.row.amount*form.rate).toFixed(2)}}
                </div>
    ...
    export default {
        ...
        methods:{
            getSummaries(param) {
                ...
                if (!values.every(value => isNaN(value)) && (column.property =='totalAmount' || column.property =='conversion')){
