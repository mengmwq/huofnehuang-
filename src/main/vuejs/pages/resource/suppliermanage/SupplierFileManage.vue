  ...
  export default {
    ...
    methods: {
      ...
      loadData () {
        ...
        // 客户附件列表
        if (path === '/router/customerFileManage') {
          let params = {
            id: id,
            name: this.query.name
          }

          Api.selectCustomerAttachments(params).then((res) => {
            if (res) {
              let _this = this
              this.listData = res
        ...
        // 合作伙伴附件列表
        if (path === '/router/partnerFileManage') {
          let params = {
            id: id,
            name: this.query.name
          }
        ...
        // 供应商附件列表
        if (path === '/router/supplierFileManage') {
          let params = {
         ...
         // 外工供应商附件列表
        if (path === '/router/outerWorkerFileManage') {
          let params = {
            supplierId:id,
            attachmentName: this.query.name
          }

          Api.selectSupplierAttachmentsOuterWorker(params).then((res) => {
            if (res) {
              let _this = this
              this.listData = res.records
              this.listData.forEach(function (item) {
