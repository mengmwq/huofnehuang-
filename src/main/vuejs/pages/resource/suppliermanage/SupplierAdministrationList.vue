  ...
  <div>
    ...
    >
      <el-table-column v-for="(header, index) in tableHeader" v-if="header.show" :key="index" :prop="header.prop"  :label="header.label" :sortable="header.sortable"
        ...
        <template slot-scope="scope">
          <div v-if="header.prop == 'name'">
            <router-link :to="{path: '/router/supplierDetail',  query: {id: scope.row.id,type:'3',supplierTypes:'1'}}">{{scope.row.name}}</router-link>
          </div>
      ...
      <el-table-column label="操作" width="150" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path: '/router/editSupplier', query: {id: scope.row.id,type:'3',supplierTypes:'1'}}" v-showBtn data-authorityType="edit-btn">
