<template>
  <div>
    ...
    >
      <el-table-column v-for="(header, index) in tableHeader" v-if="header.show" :fixed="header.fixed" :key="index" :prop="header.prop"  :label="header.label" :sortable="header.sortable"
                       :align="header.align" :min-width="header.width" :column-key="header.prop" show-overflow-tooltip>
        <template slot-scope="scope">
          <div v-if="header.prop == 'name'">
            <router-link :to="{path: '/router/supplierOuterWorkerDetails', query: {id: scope.row.id,type:'2'}}" v-showBtn data-authorityType="edit-btn">{{scope.row.name}}</router-link>
          </div>
      ...
      <el-table-column label="操作" width="150" fixed="right">
        <template slot-scope="scope">
          <router-link :to="{path: '/router/editOuterWorkerSupplier', query: {id: scope.row.id,type:'2'}}" v-showBtn data-authorityType="edit-btn">
  ...
  export default {
    ...
    methods: {
      ...
      loadData (pageNum=1) {
        ...
        let _this = this
        Api.supplierListOuterWorker(this.query).then(res => {
      ...
      deleteItem (item) {
        this.$confirm('您确定要删除【 ' + item.name + ' 】吗?', '提示', {
          ...
          Api.deleteSupplierOuterWorker({id: item.id}).then((res) => {
            if (res) {
              this.$notify.success({
                title: '提示',
                message: '删除成功'
